...
Both are decided by parsing the command as shell, not by matching the command string — a commit message that talks about "npm … publish" is data, and never reads as a publish.
...
The same checks, run when you want them instead of on a command: sensitive tracked files, secrets in staged/unstaged/untracked changes, and tarball contents plus a secret scan for **every non-private package in the workspace** (which the per-command hook can't cover for `pnpm -r publish`). Prints one compact report and always exits 0. The plugin's `bin/` is on PATH automatically, so the command works from any project with the plugin installed.
